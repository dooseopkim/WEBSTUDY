...
public class TodoDao {
	...
	public int addTodo(TodoDto todoDto) {
		...
		int addCount = -1;
		try (PreparedStatement ps = TodoConnection.getConnection().prepareStatement(addTodoSql)) {
	...
	public List<TodoDto> getTodos() {
		...
		String getTodosSql = "SELECT `id`, `title`, `name`, `sequence`, `type`, `regdate` " + "FROM `todo` ORDER BY `regdate` ASC";

		try (ResultSet rs = TodoConnection.getConnection().prepareStatement(getTodosSql).executeQuery()) {
			while (rs.next()) {
	...
	public int updateTodo(TodoDto todoDto) {
		...
		int updateCount = -1;
		try (PreparedStatement ps = TodoConnection.getConnection().prepareStatement(updateTodoSql)) {
